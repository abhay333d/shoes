...
const PoseFlipFilter = (function () {
  ...
  function compute_dpMax(ipp0, ipp1) {
    ...
    const n = ipp0.length;
    for (let i = 0; i < n; ++i) {
      const dx = ipp0[i][0] - ipp1[i][0],
        dy = ipp0[i][1] - ipp1[i][1];
  ...
  const superThat = {
    instance: function (spec) {
      ...
      function compute_objectBoundingSphereRadius(threeObject) {
        ...
        for (
          let i = threeObjectWithoutOccluders.children.length - 1;
          i >= 0;
          --i
        ) {
      ...
      function compute_apparentDiameterPx(
        radius,
        threeMatMov,
        threeCamera,
        viewHeightPx
      ...
      const that = {
        ...
        update: function (
          ...
          if (++_resetCounter <= _spec.startStabilizeCounter) {
            _previous.imagePointPositions =
              clone_imagePointPositions(imagePointPositions);
            _previous.threeQuaternion.copy(_threeQuaternion);
            _previous.threePosition.copy(_threePosition);

            if (_boundingSphereRadius < 0) {
              // costly to compute, we should not do it often
              _boundingSphereRadius =
                compute_objectBoundingSphereRadius(threeObject);
            ...
            const dp = Math.max(
              _spec.dpMinPx,
              compute_dpMax(imagePointPositions, _previous.imagePointPositions)
            );
            ...
            const apparentDiameterPx = compute_apparentDiameterPx(
              _boundingSphereRadius,
              threeMatMov,
              threeCamera,
              viewHeightPx
            ...
            const dPixTrans =
              (apparentDiameterPx / (2 * _boundingSphereRadius)) * dPos;

            // APPLY FILTERING:
            // compute amortization factors, between 0 (full amortization) and 1 (no amortization)
            const rotFactor = Math.min(
              1,
              (_epsilon + dp) / (_epsilon + dPixRot / _spec.dPixRotTol)
            );
            const posFactor = Math.min(
              1,
              (_epsilon + dp) / (_epsilon + dPixTrans / _spec.dPixTransTol)
            );
            ...
            _threeQuaternionApplied.slerpQuaternions(
              _previous.threeQuaternion,
              _threeQuaternion,
              rotFactor
            );
            _threePositionApplied.lerpVectors(
              _previous.threePosition,
              _threePosition,
              posFactor
            );

            // save image point positions for next iteration:
            copy_imagePointPositions(
              imagePointPositions,
              _previous.imagePointPositions
            );
            ...
            // build output matrix:
            _threeFilteredMatMov.compose(
              _threePositionApplied,
              _threeQuaternionApplied,
              _threeScale
            );
