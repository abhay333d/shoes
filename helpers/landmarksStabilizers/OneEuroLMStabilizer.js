...
function OneEuroFilter(spec) {
  ...
  function compute_alpha(cutoff, freq) {
  ...
  this.filter = function (v, beta, freq) {
    const dvalue = _x.has_lastRawValue()
      ? (v - _x.get_lastRawValue()) * freq
      : 0.0;
    const edvalue = _dx.filter_withAlpha(
      dvalue,
      compute_alpha(spec.dcutoff, freq)
    );
      ...
      const that = {
        update: function (landmarks, widthPx, heightPx, scale) {
          ...
          if (_lastTime !== -1) {
            ...
            // clamp freq:
            _freq = Math.min(
              Math.max(_freq, _spec.freqRange[0]),
              _spec.freqRange[1]
            );
          }
          _lastTime = timestamp;
          ...
          for (let i = 0; i < LMCount; ++i) {
            ...
            const dx = compute_distanceNNInput(
              x,
              xStab,
              _spec.NNInputSizePx,
              scale
            );
            const dy = compute_distanceNNInput(
              y,
              yStab,
              _spec.NNInputSizePx,
              scale * aspectRatio
