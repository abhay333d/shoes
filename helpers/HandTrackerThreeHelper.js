...
const HandTrackerThreeHelper = (function () {
  ...
  function init_landmarksStabilizers(landmarksStabilizerSpec) {
    ...
    for (let i = 0; i < _spec.maxHandsDetected; ++i) {
      stabilizers.push(
        WebARRocksLMStabilizer.instance(landmarksStabilizerSpec)
      );
    }
    return stabilizers;
  }
      ...
      const add_threeObjectToParent = function (threeParents, threeChild) {
        for (let i = 0; i < _spec.maxHandsDetected; ++i) {
          ...
          threeChild.traverse(function (threeStuff) {
            isChildContainsSkinnedMesh =
              isChildContainsSkinnedMesh || threeStuff.isSkinnedMesh;
          });
          ...
          const threeChildCopy =
            isChildContainsSkinnedMesh || !_spec.enableFlipObject
              ? threeChild
              : threeChild.clone();
